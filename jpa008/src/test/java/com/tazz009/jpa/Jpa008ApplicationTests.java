...
public class Jpa008ApplicationTests {
...
//	@Transactional
	...
	@Test
	public void testC_RFIND() throws Exception {
		Product productA = Product.builder().id("productA").name("상품A").build();
		Optional<Product> findedProduct = productService.findProductById(productA.getId());
		
		assertThat(findedProduct.get().getId(), is(productA.getId()));
		assertThat(findedProduct.get().getName(), is(productA.getName()));
		assertThat(findedProduct.get().getMembers().get(0).getUsername(), is("회원1"));
	}
